...
运行时依赖只有 `ifaddr`，用于枚举本机网络接口。V0.2.0 Alpha 9 不使用 `python-zeroconf` 或 mDNS 发布逻辑。
...
配置写入 `%LOCALAPPDATA%\Pigeon\config.json`。程序仅在迁移时读取旧名称目录中的配置。测试或截图前不要把真实 token 放进仓库。
...
```powershell
.venv\Scripts\pyinstaller --noconfirm --clean pigeon.spec
```

输出位于 `dist\Pigeon.exe`。发布包还必须包含：
...
3. Windows 网络类别为“专用”，Pigeon 防火墙只勾选“专用”。
